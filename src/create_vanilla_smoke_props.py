"""Vanilla-only, static tip wisps; alpha-tested mesh, NOT a particle system.

Run with smoking_animation_work.blend. The inventory meshes remain unchanged.
Kenshi's stock ItemShader.ALPHA clips normal-map alpha: use deterministic
coverage dithering, not a continuous alpha value that would simply disappear.
"""
...
diffuse=[];normal=[];rng=random.Random(420)
for y in range(height):
    for x in range(width):
        if x<512:
            ...
            spread=.07+.09*v
            density=.3*math.exp(-((u-center)/spread)**2)*math.sin(math.pi*v)**.7
            alpha=1.0 if rng.random()<density else 0.0
            diffuse.extend([.55,.57,.59,0]);normal.extend([.5,.5,1,alpha])
...
for kind,tip in [('chillum',1.79),('joint',1.23)]:
    ...
    for cross in [side,across]:
        start=len(vertices)
        for j in range(17):
            t=j/16;center=Vector((0,0,tip))+up*(.9*t)+side*(.065*math.sin(t*8)*t)
            for sign,u in [(-1,.501),(1,.999)]:
                vertices.append(center+cross*(sign*(.08+.15*t)))
                coords.append((u,.002+.996*t))
        for j in range(16):
            a=start+j*2;faces.extend([(a,a+1,a+3,a+2),(a+2,a+3,a+1,a)])
    ...
    wisp.data.materials.append(obj.data.materials[0])
    bpy.ops.object.select_all(action='DESELECT');obj.select_set(True);wisp.select_set(True)
    ...
    report[kind]={'mesh':obj.name+'.mesh','tip_local_z':tip,'added_faces':len(faces),'mode':'static alpha-tested wisp, no particles'}
