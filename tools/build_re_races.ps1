...
$outPath = Join-Path $rootPath 'build\420_Races'
$objPath = Join-Path $rootPath 'build\re_races_obj'
New-Item -ItemType Directory -Force $outPath,$objPath | Out-Null
& "$vc64\bin\amd64\cl.exe" /nologo /LD /MD /EHsc /O2 /GL /W3 /wd4819 /DNDEBUG /DUNICODE /D_UNICODE /DBOOST_ALL_NO_LIB /DBOOST_ERROR_CODE_HEADER_ONLY /D_WIN32_WINNT=0x0601 'src\re_smoke\SmokingRaces.cpp' "/Fo$objPath\SmokingRaces.obj" /link /LTCG "/OUT:$outPath\SmokingRaces.dll" "/IMPLIB:$objPath\SmokingRaces.lib" KenshiLib.lib OgreMain_x64.lib kernel32.lib user32.lib
if ($LASTEXITCODE -ne 0) { throw "Compiler exited with $LASTEXITCODE" }
& "$vc64\bin\amd64\dumpbin.exe" /exports "$outPath\SmokingRaces.dll"
